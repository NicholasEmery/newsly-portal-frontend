...
const AccordionComponent: React.FC<NoticeComponentProps> = ({
  ...
  return (
    <section className="my-8 rounded-[32px] bg-white px-6 py-5 md:px-12 md:py-9">
      {items.map((item, index) => {
        ...
        return (
          <div
            ...
            className="border-b border-[#d7d7d7] py-4 md:py-6"
          >
            <button
              type="button"
            ...
            >
              <span className="pr-4 text-[30px] font-semibold leading-tight text-[#131722] md:text-[44px]">
                {item.title}
              </span>
              <span
                className={`mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border text-2xl leading-none transition-colors ${
                  isOpen
                    ...
                    : "border-[#d0d0d0] bg-transparent text-[#121722]"
                }`}
              >
                {isOpen ? "-" : "+"}
              </span>
            ...
            >
              <div className="overflow-hidden">
                <div
                  className={`max-w-[900px] pt-4 text-[24px] leading-[1.55] text-[#3b3f49] transition-all duration-300 md:pt-6 ${
                    isOpen
