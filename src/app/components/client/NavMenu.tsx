...
import { useNavPin } from "@/app/hooks/useNavPin";
...
const NavMenu = ({ categories }: NavMenuProps) => {
  ...
  const { navRef, pinned, slideIn, navHeight, scrollDirDown } = useNavPin();
  ...
  return (
    <>
      ...
      >
        ...
        >
          <ul className="min-w-max w-full flex flex-row items-center md:justify-center justify-start gap-7 lg:gap-10">
            ...
            >
              <button
                type="button"
                className="group flex items-center uppercase cursor-pointer outline-none focus:outline-none"
        ...
        <div
          className={`transition-all duration-500 ease-in-out ${openSearchBar ? "lg:w-auto" : "w-6 justify-end lg:justify-center lg:w-auto"} ${pinned ? "flex" : "hidden"}`}
        >
          <ThemeModeBtn />
          <div
